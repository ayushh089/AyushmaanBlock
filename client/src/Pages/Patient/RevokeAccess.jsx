...
const RevokeAccess = () => {
  ...
  const RevokeAccess = async () => {
    if (contract) {
      console.log("Revokeing access to:", walletAddress);
      try {
        setLoading(true);
        const tx = await contract.revokeAccess(walletAddress);
        await tx.wait();
        toast.success("Access Revoked successfully");
        // alert("Access Revokeed successfully");
        setWalletAddress("");
        setLoading(false);
      } catch (error) {
        if (error.reason) {
  ...
  return (
    <div className="bg-gray-100 flex flex-col items-center justify-center p-4 w-full">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Revoke Access</h1>
      <form
